...
export function renderExamEnginePage(container, currentUser, onFinishExam) {
    ...
    // Render outer shell once so header & timer container remain persistent in DOM
    container.innerHTML = `
        <div class="min-h-screen bg-slate-100 flex flex-col justify-between">
            
            <!-- CBT Top Header Bar (Persistent) -->
                        ...
                        <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    <span id="cbt-question-counter" class="text-xs font-semibold text-slate-500">Soal 1 / ${questions.length}</span>
                                </div>
            ...
            <!-- Dynamic Question Content & Controls -->
            <div id="cbt-body-shell" class="flex-grow flex flex-col justify-between"></div>

        </div>
    `;

    // Start timer ONCE and keep it running continuously throughout the entire exam
    ...
    function renderCBTBody() {
        const bodyShell = document.getElementById('cbt-body-shell');
        const counterEl = document.getElementById('cbt-question-counter');
        if (!bodyShell) return;
        ...
        if (counterEl) {
            counterEl.textContent = `Soal ${currentIndex + 1} / ${questions.length}`;
        }
        ...
        bodyShell.querySelectorAll('.option-card').forEach(card => {
        ...
        if (finishBtn) {
            finishBtn.addEventListener('click', () => {
                ...
                renderFinishConfirmationModal(questions.length - ansCount, questions.length, () => {
                    cleanup();
                    const result = storageService.finishExamSession();
                    if (onFinishExam) onFinishExam(result);
                });
